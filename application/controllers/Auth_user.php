...
class Auth_user extends CI_Controller {
	...
	public function index()
	{
        ...
        $this->load->view('home/elemen_login/header_login');
        $this->load->view('home/login_user');
        $this->load->view('home/elemen_login/footer_login');
    }
    ...
    public function proses()
    {
        ...
        if(isset($post['login_user']))
        {
            ...
            $query = $this->Customer_m->login_users($post);
            if($query->num_rows() > 0){
                $row = $query->row();
                $params  = array (
    ...
    {
        $params		=	array('customerid', 'name', 'gender', 'email', 'address');
